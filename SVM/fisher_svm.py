...
for k in range(10,25):
    ...
    kbest_preds = svm.predict(selected_features_test)

    mae = round(mean_absolute_error(target_test, kbest_preds), 3)
    mae_list.append(mae)

    rmse = round(math.sqrt(mean_squared_error(target_test, kbest_preds)), 3)
    rmse_list.append(rmse)

    r2 = round(r2_score(target_test, kbest_preds), 6)
    r2_list.append(r2)
...
selector = SelectKBest(score_func=f_regression, k=k_list[min_index_mae])
selector.fit(features_train_scaled, target_train)
...
selector = SelectKBest(score_func=f_regression, k=k_list[min_index_rmse])
selector.fit(features_train_scaled, target_train)
...
selector = SelectKBest(score_func=f_regression, k=k_list[max_index_r2])
selector.fit(features_train_scaled, target_train)
